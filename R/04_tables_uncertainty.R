...
relahc <- read_xlsx("data/Poverty 3yr Scotland.xlsm", sheet = "relahc_main") %>%
  mutate(type = "rel", key = "ahc") %>%
  select(-"0")

relbhc <- read_xlsx("data/Poverty 3yr Scotland.xlsm", sheet = "relbhc_main") %>%
  mutate(type = "rel", key = "bhc") %>%
  select(-"0")

absahc <- read_xlsx("data/Poverty 3yr Scotland.xlsm", sheet = "absahc_main") %>%
  mutate(type = "abs", key = "ahc") %>%
  select(-"0")

absbhc <- read_xlsx("data/Poverty 3yr Scotland.xlsm", sheet = "absbhc_main") %>%
  mutate(type = "abs", key = "bhc") %>%
  select(-"0")

cmdbhc <- read_xlsx("data/LowincMatDepCh 3yr Scotland.xlsm", sheet = "main") %>%
  mutate(type = "cmd", key = "bhc", thresh = NA) %>% head(1L)

penmd <- read_xlsx("data/MatDepPn 3yr Scotland.xlsm", sheet = "main") %>%
