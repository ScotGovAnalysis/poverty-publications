...
filename <- "output/data2022.xlsx"
mytables <- readRDS("data/tables.rds")

# final three tables are income distribution table and equivalence scales,
# not to be included in spreadsheet
mytables <- head(mytables, n = -3)
...
myheaders <- list(
  ...
  list(header = c("Gender of single adults",
                  "This worksheet contains seven tables.",
                  "Important: The latest estimate is unreliable, please do not use. See README for more information.",
                  "Note: The term 'single' here refers to adults who are sharing a household with no other adults. This differs from the analysis in the 'Family type' worksheet, where single adults may share the household with other families.",
                  "Source: Scottish Government analysis of the Family Resources Survey"),
  ...
  list(header = c("Household food security levels (2-year average)",
                  "This worksheet contains three tables.",
                  "Important: The latest estimate is unreliable, please do not use. See README for more information.",
                  "Note: Food security questions were newly added to the Family Resources Survey in 2019/20. Therefore, only a two-year average is available. The questions ask about whether people were worried about running out of food, had to reduce meal sizes or skip meals.",
                  "Note: Shared households such as such as a group of students, or other unrelated adults were excluded from the analysis",
                  "Source: Scottish Government analysis of the Family Resources Survey"),
       titles = c("Proportion of people in each household food security category, Scotland 2019/20-2020/21",
                  "Number of people in each household food security category, Scotland 2019/20-2020/21",
                  "Number of families in each group in the combined two-year survey sample, Scotland 2019/20-2020/21")),

  # * -- child poverty characteristics -----------------------------------------
  ...
  list(header = c("Ethnicity (3-year average)",
       ...
       titles = c("Relative poverty rate: Proportion of children in each category who are in relative poverty (below 60% of UK median income after housing costs), Scotland",
                  ...
                  "Age: Median age of family head (families with children) in each category, Scotland",
                  "Sample size: Number of families with children in each category in the combined three-year survey sample, Scotland")),
  list(header = c("Children's household food security levels (2-year average)",
                  "This worksheet contains three tables.",
                  "Important: The latest estimate is unreliable, please do not use. See README for more information.",
                  "Note: Food security questions were newly added to the Family Resources Survey in 2019/20. Therefore, only a two-year average is available. The questions ask about whether people were worried about running out of food, had to reduce meal sizes or skip meals.",
                  "Note: Shared households such as such as a group of students, or other unrelated adults were excluded from the analysis",
                  "Source: Scottish Government analysis of the Family Resources Survey"),
       titles = c("Proportion of children in each household food security category, Scotland 2019/20-2020/21",
                  "Number of children in each household food security category, Scotland 2019/20-2020/21",
                  "Sample size: Number of families with children in each group in the combined two-year survey sample, Scotland 2019/20-2020/21")),
...
mark_list <- list(
  ...
  "7" = list(markers = c("[x]", "[x]", "[b]", "[x]", "[x]", "[b]", "[x]", "[x]",
                         "[b]"),
             ranges = c("B8:F9", "H10:P11", "G8:G11", "B14:F15", "H16:P17",
                        "G14:G17", "B20:F20", "H21:P21", "G20:G21")),

  # disability
  "18" = list(markers = c(rep("[x]", 7), rep("[b]", 2)),
              ranges = c("B11:B14", "B20:B23", "B29:B32", "B38:B42", "B47:B50",
                         "B56:B59", "B65:B69", "R29:S34", "R56:S61")),
  "19" = list(markers = c(rep("[x]", 7), rep("[b]", 2)),
              ranges = c("B12:B15", "B21:B24", "B30:B33", "B39:B43", "B48:B51",
                         "B57:B60", "B66:B70", "R30:S35", "R57:S62")),

  # babies; young mothers
  "26" = list(markers = rep("[x]", 7),
              ranges = c("H9:O10", "H14:O15", "H19:O20", "H24:O25", "H29:O30",
                         "H34:O35", "H39:O40")),
  "27" = list(markers = rep("[x]", 7),
              ranges = c("E9:L10", "E14:L15", "E19:L20", "E24:L25", "E29:L30",
                         "E34:L35", "E39:L40")),
  # disability
  "32" = list(markers = c(rep("[x]", 7), rep("[b]", 2)),
              ranges = c("B11:B14", "B20:B23", "B29:B32", "B38:B42", "B47:B50",
                         "B56:B59", "B65:B69", "R29:S34", "R56:S61")),
  "33" = list(markers = c(rep("[x]", 7), rep("[b]", 2)),
              ranges = c("B12:B15", "B21:B24", "B30:B33", "B39:B43", "B48:B51",
                         "B57:B60", "B66:B70", "R30:S35", "R57:S62"))
  )
...
createContentSheet(filename = filename,
                   ...
                   headings = list(titles = c("H2-Headline poverty measures - after housing costs",
                                              ...
                                              "H2-Poverty characteristics - relative and severe poverty after housing costs",
                                              "H2-Child poverty characteristics - relative and severe poverty after housing costs",
                                              "H2-Income and income inequality"),
                                   location = c(0, 3, 6, 8, 22, 37)))
...
createReadmeSheet(
  ...
  notes = list(
    "H1-Important notes" = c("Published: 31 March 2022",
                             ...
                             "The tables in this spreadsheet contain all estimates shown in the 'Poverty and Income Inequality in Scotland' analytical report, and further breakdowns. The report can be found here: https://data.gov.scot/poverty/2022",
    ...
    "H2-COVID-19 impact" = "These are not official statistics. The latest estimates (all estimates which include data from 2020/21, for example 2018-21, and 2016-21) are unreliable as they are based on data collected during the first year of the coronavirus (COVID-19) pandemic. Lockdown rules severely disrupted the data collection. As a result, we were unable to obtain a representative sample for Scotland. Please refer to Poverty and Income Inequality in Scotland 2017-20 for the latest National Statistics on poverty and income inequality. These tables are made available in the interest of transparency.",
    ...
    "H2-Poverty measures" = c("In this spreadsheet, we use different measures of poverty. Each measures a different aspect of people's living standards.",
                              ...
                              "The food security measure looks at how many people worry about running out of food or have to skip meals. Note that we measure food security on a household level, but not everyone in the household always experiences this in the same way. For example, a parent may reduce their own meal sizes but not those of their young children."),
    ...
    "H2-Housing costs" = c("The most commonly used poverty indicator in Scotland is relative poverty after housing costs. After-housing-costs measures describe disposable household income after housing costs are paid for. They therefore better describe what is available to spend on food, bills and leisure every week or month. ",
                           "The poverty characteristics tables are only available for poverty after housing costs."),
