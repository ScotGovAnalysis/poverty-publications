...
#### Figure 28 - Median weekly household income in `r params$data_year` prices, Scotland
```{r chart28}
...
#### Figure 29 - Weekly household income before housing costs at each decile point in `r params$data_year` prices, Scotland
```{r chart29}
interactive(povertycharts[["chart29"]])
```
...
#### Figure 30 - Equivalised weekly household income distribution before housing costs, Scotland `r params$data_period`
```{r chart30}
interactive(povertycharts[["chart30"]])
```
...
#### Table 1 - Income and poverty thresholds for different household types before housing costs, Scotland `r params$data_period`
```{r table1}
...
#### Figure 31 - Income sources as a percentage of gross income by income decile, Scotland `r params$data_period`
```{r chart31}
interactive(povertycharts[["chart31"]], height = 5)
```
