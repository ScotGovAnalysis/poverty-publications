...
```{r}
keymessage <- "Median weekly household income growing"
message(keymessage)
```
...
subtitle <- str_c("Median weekly household income in ", params$data_year, 
                  " prices, Scotland")
interactive(povertycharts[["chart28"]], title = title,
            subtitle = subtitle)
```

```{r}
keymessage <- "Lower incomes increasing more than higher incomes"
message(keymessage)
```
...
interactive(povertycharts[["chart29"]], height = 3.5, title = title,
            subtitle = subtitle)
```
...
subtitle <- str_c("Equivalised weekly household income distribution before housing costs, Scotland ", 
                 params$data_period)
interactive(povertycharts[["chart30"]], title = title,
            subtitle = subtitle)
```
...
subtitle <- str_c("Income sources as a percentage of gross income by income decile, Scotland ", 
                 params$data_period)
interactive(povertycharts[["chart31"]], height = 5.5, title = title,
            subtitle = subtitle)
```
