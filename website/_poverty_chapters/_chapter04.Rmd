...
The Child Poverty (Scotland) Act 2017 requires us to report annually on a number of child poverty measures. The latest single-year estimates can be found in the [Child poverty summary](https://data.gov.scot/poverty/cpupdate.html). The analysis in this report however includes three-year averaged estimates only, which are more suitable for showing trends.
...
```{r}
keymessage <- "Relative poverty rate for children no longer rising"
message(keymessage)
```
...
```{r}
keymessage <- "Absolute poverty rate for children stable"
message(keymessage)
```
...
interactive(povertycharts[["chart11"]], height = 4.5, title = title, 
            subtitle = subtitle)
```
...
interactive(povertycharts[["chart12"]], height = 3.5, title = title, subtitle = subtitle)
```
...
```{r}
keymessage <- "Share of children in poverty who live in working households no longer rising"
message(keymessage)
```
...
subtitle <- "Proportion of children in relative poverty who live in working households, Scotland"
interactive(povertycharts[["chart13"]], height = 3.5, title = title, subtitle = subtitle)
```

#### Food security

```{r}
keymessage <- "Children in poverty often lack food security"
message(keymessage)
```

Add stats here

`r infobox("Food security questions were newly added to the Family Resources Survey in 2019/20. They ask about whether people were worried about running out of food, had to reduce meal sizes or skip meals. More information can be found in the [Definitions](#food-security-2) section.")`
...
Some types of households are known to be at a particularly high risk of child poverty. These include households with lone parents, three or more children, disabled household members, of a minority ethnic background, with a child under one, or a mother under 25. These groups do not cover everyone at higher risk of poverty, but taken together, they cover the majority of households with children that are in poverty.

`r infobox("Here, the minority ethnic group includes white minorities such as white people who are not British.")`
...
subtitle <- "Proportion of children in relative poverty after housing cost, Scotland"
...
subtitle <- "Proportion of children in absolute poverty after housing costs, Scotland"
