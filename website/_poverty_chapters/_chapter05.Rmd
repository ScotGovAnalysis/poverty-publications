...
```{r chart18}
caption <- "Figure 18: Proportion of adults in relative poverty after housing costs, Scotland"
interactive(povertycharts[["chart18"]], caption = caption)
```
...
`r infobox("In this publication, 'child' refers to a dependent child living in the household. This is explained in the [Definitions section](#child).\n\nEstimates for single fathers are not available due to small sample sizes.")`

```{r chart19}
caption <- "Figure 19: Proportion of single working-age adults in relative poverty after housing costs, Scotland"
interactive(povertycharts[["chart19"]], height = 4, caption = caption)
```
...
```{r chart20}
caption <- "Figure 20: Proportion of single pensioners in relative poverty after housing costs, Scotland"
interactive(povertycharts[["chart20"]], height = 4, caption = caption)
```
...
`r infobox("By 'Single' we mean adults who have never been married or in a Civil Partnership, and are not living with their partner.\n\nThe 'Married' category includes Civil Partnerships, and couples who are married or in a Civil Partnership but temporarily living apart.\n\nThe 'Divorced' category includes divorced couples, dissolved Civil Partnerships, and couples who are married or in a Civil partnership but are not living together because of estrangement.")`

```{r chart21}
caption <- "Figure 21: Proportion of adults in relative poverty after housing costs, Scotland"
interactive(povertycharts[["chart21"]], height = 4, caption = caption)
```
...
`r infobox("This analysis doesn't take into account differences in the age profiles of the ethnic groups.\n\nFor the 'White - British' ethnic group the median average age of the highest income earner was 50 compared with a median age of 36 for the 'White - Other' ethnic group, 39 for 'Asian or Asian British' and and 40 for 'Mixed, Black, Black British or Other' ethnic groups.\n\nOlder people have a lower poverty rate, so the age profile partly explains the lower poverty rate for the 'White - British' ethnic group. However, the age difference cannot explain the entire gap in poverty rates between ethnic groups.")`

```{r chart22}
caption <- "Figure 22: Proportion of people in relative poverty after housing costs, Scotland 2014-2019"
interactive(povertycharts[["chart22"]], caption = caption)
```
...
`r infobox("This analysis doesn't take into account differences in the age profiles of the religions.\n\nFor adults belonging to the Church of Scotland, the median average age was 61. In contrast, the median age was 35 for Muslim adults, and 40 for adults belonging to no religion.\n\nOlder people have a lower poverty rate, so age profile partly explains the lower poverty rate for people belonging to the Church of Scotland. However, the age difference cannot explain the entire gap in poverty rates between religious groups.")`

```{r chart23}
caption <- "Figure 23: Proportion of adults in relative poverty after housing costs, Scotland 2014-19"
interactive(povertycharts[["chart23"]], caption = caption)
```
...
```{r chart25}
caption <- "Figure 25: Proportion of people in relative poverty (disability benefits removed from household income) after housing costs, Scotland"
interactive(povertycharts[["chart25"]], caption = caption)
```

`r infobox("Excluding disability benefits from the analysis changes the poverty threshold. This is because some households now move down the income distribution, which lowers the median income and therefore also the poverty threshold. As a consequence, some households without a disabled person that were just below the poverty threshold in the previous analysis will now be above the threshold.\n\nAs a result, the poverty rate for households without a disabled person is slightly lower.")`
